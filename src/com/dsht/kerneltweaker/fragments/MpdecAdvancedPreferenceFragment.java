...
public class MpdecAdvancedPreferenceFragment extends PreferenceFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(R.xml.pref_screen_mpdec_advanced);
		mCategory = (PreferenceCategory) findPreference("key_mpdecision_cat");
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		String curGov = Helpers.getFileContent(new File(MPDECISION_FILE)).trim().replaceAll("\n", "");
		File f = new File("/sys/kernel/msm_mpdecision/conf/");
		if(f.exists()) {
			MainActivity.menu.setEnabled(false);
			mCategory.setTitle(" Tweakable values");
			addPreferences();
		}
		else {
			CustomPreference pref = new CustomPreference(mContext, true, category);
			pref.setTitle("No tweakable values");
			pref.setSummary(" Doesn\'t have tweakable values");
	...
	public static void addPreferences() {
		...
		class LongOperation extends AsyncTask<String, Void, String> {
			...
			protected String doInBackground(String... params) {
				...
				String currentGovernor = Helpers.readFileViaShell(MPDECISION_FILE, true).split("\n")[0];
				File f = new File("/sys/kernel/msm_mpdecision/conf/");
				if(f.exists()) {
